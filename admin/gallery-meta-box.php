...
function add_gallery_img_func(){
    add_meta_box(
        ...
        array('property', 'property-commercial'), // Change post type name
        'normal',
        'core'
    );
}
...
function gallery_print_func()
{
    ...
    if( !in_array($post->post_type, $property_types) )
        return;
    ?>
    <style type="text/css">
        .gallery_area_div {
...
function properties_save( $post_id ) {
    ...
    // Correct post type
    $property_types = array('property', 'property-commercial');
    if ( !isset($_POST['post_type']) || !in_array($_POST['post_type'], $property_types ) )
        return;
